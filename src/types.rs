...
pub struct TxRow {
    ...
    /// Inner calls of an AA (type 0x76) transaction, in order. Empty for
    /// non-AA txs. Persisted to the Postgres `tx_calls` table only when
    /// there is more than one call — the AA envelope mirrors calls[0] in
    /// `to`/`value`/`input` — and serialized whole into the ClickHouse
    /// `calls` JSON column.
    pub calls: Vec<Call>,
    pub call_count: i16,
