...
pub fn run(args: Args) -> Result<()> {
    ...
    let enable_pg_duckdb = Confirm::new()
        .with_prompt("Enable pg_duckdb for analytics? (requires pg_duckdb extension)")
        .default(true)
        .interact()?;
    ...
    let config = generate_config(&name, chain_id, &rpc_url, &pg_url, enable_pg_duckdb, http_port);
...
fn generate_config(
    ...
    rpc_url: &str,
    pg_url: &str,
    enable_pg_duckdb: bool,
    http_port: u16,
) -> String {
    let analytics_engine = if enable_pg_duckdb { "pg_duckdb" } else { "postgres" };
