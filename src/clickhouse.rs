...
impl ClickHouseEngine {
    ...
    /// Return the URL of the ClickHouse instance.
    pub fn url(&self) -> &str {
        &self.url
    }

    /// Execute a query and return results as JSON values.
    pub async fn query(&self, sql: &str, signature: Option<&str>) -> Result<QueryResult> {
        ...
        self.try_query(&sql, start).await
    }

    async fn try_query(&self, sql: &str, start: std::time::Instant) -> Result<QueryResult> {
        let url = format!(
