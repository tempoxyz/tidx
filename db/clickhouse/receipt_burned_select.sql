-- Decodes TIP-1028 `ReceiptBurned(address indexed token,
...
-- address recoveryAuthority, address caller, uint256 amount)` emitted by the
-- ReceivePolicyGuard precompile when a held receipt is burned (not claimed).
--
-- Identical layout to ReceiptClaimed minus the `to` word:
--   word0 blockedAt      uint64  (hex 51..66)
--   word1 receiptVersion uint8   (hex 129..130)
--   word2 originator     address (hex 155..194)
--   word3 recipient      address (hex 219..258)
--   word4 recoveryAuth   address (hex 283..322)
--   word5 caller         address (hex 347..386)
--   word6 amount         uint256 (hex 387..450)
SELECT
    block_num,
    block_timestamp,
    tx_idx,
    ...
    reinterpretAsUInt256(reverse(unhex(substring(data, 387, 64)))) AS amount
FROM logs
WHERE
    selector = '0x61d14663748cd685c80a8434fc260ea4f4b27e213768b94375aa33b3985bf952'
