...
#[tokio::test]
#[serial(clickhouse)]
async fn test_create_virtual_deposits_view() {
    let ch = TestClickHouse::new(TEST_DB)
        .await
        .expect("Failed to create ClickHouse client");
    ...
    let sink = ClickHouseSink::new(&ch.url, TEST_DB, None, None).expect("sink");
    sink.ensure_schema().await.expect("schema");

    let token = vec![0xdau8; 20];
    let sender = vec![0x11u8; 20];
    let master = vec![0x22u8; 20];
    let virtual_address = hex::decode("01020304fdfdfdfdfdfdfdfdfdfda1b2c3d4e5f6").unwrap();
    let transfer_selector = hex::decode(
        "ddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef",
    )
    .unwrap();

    let pad_address = |addr: &[u8]| {
        std::iter::repeat_n(0u8, 12)
            .chain(addr.iter().copied())
            .collect::<Vec<_>>()
    };
    let amount_data = |amount: u128| {
    ...
    sink.write_logs(&[
        LogRow {
            ...
            address: token.clone(),
            selector: Some(transfer_selector.clone()),
            topic0: Some(transfer_selector.clone()),
            topic1: Some(pad_address(&sender)),
            topic2: Some(pad_address(&virtual_address)),
            topic3: None,
        ...
        LogRow {
            ...
            selector: Some(transfer_selector.clone()),
            topic0: Some(transfer_selector.clone()),
            topic1: Some(pad_address(&virtual_address)),
            topic2: Some(pad_address(&master)),
            topic3: None,
            data: amount_data(100),
    ...
    .expect("create table");
    ch.query(&format!(
        "CREATE MATERIALIZED VIEW virtual_deposits_mv TO virtual_deposits AS {}",
        sql
    ))
    .await
    .expect("create mv");
    ch.query(&format!("INSERT INTO virtual_deposits {}", sql))
        .await
        .expect("backfill");

    let result = ch
        .query_json(
            "SELECT sender, virtual_address, master_address, master_id, user_tag, amount FROM virtual_deposits",
        )
        .await
        .expect("query view");

    let rows = result["data"].as_array().expect("rows array");
    assert_eq!(rows.len(), 1, "only virtual transfer pair should be materialized");
    let row = &rows[0];
    assert_eq!(
        row["sender"].as_str().unwrap(),
        "0x1111111111111111111111111111111111111111"
    );
    assert_eq!(
        row["virtual_address"].as_str().unwrap(),
        "0x01020304fdfdfdfdfdfdfdfdfdfda1b2c3d4e5f6"
    );
    assert_eq!(
        row["master_address"].as_str().unwrap(),
        "0x2222222222222222222222222222222222222222"
    );
    assert_eq!(row["master_id"].as_str().unwrap(), "0x01020304");
    assert_eq!(row["user_tag"].as_str().unwrap(), "0xa1b2c3d4e5f6");
    assert_eq!(row["amount"].as_str().unwrap_or("100"), "100");
}
